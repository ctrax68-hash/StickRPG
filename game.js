...
// Helper: attach both touch and click, preventing double-fire
...
addTapHandler(document.getElementById(plusId), () => {
if (allocPts <= 0) return;
allocVals[stat]++; allocPts–;
document.getElementById(valId).textContent = allocVals[stat];
document.getElementById(‘ptsLeft’).textContent = ’Points: ’ + allocPts;
});
addTapHandler(document.getElementById(minusId), () => {
if (allocVals[stat] <= 0) return;
allocVals[stat]–; allocPts++;
document.getElementById(valId).textContent = allocVals[stat];
document.getElementById(‘ptsLeft’).textContent = ’Points: ’ + allocPts;
});
...
// Continue
addTapHandler(document.getElementById(‘continueBtn’), () => {
if (loadGame()) { launchGame(false); }
else { toast(‘Save corrupted.’); }
});

// New game
addTapHandler(document.getElementById(‘startBtn’), () => {
...
// Delete save
addTapHandler(document.getElementById(‘delSave’), () => {
deleteSaveData();
