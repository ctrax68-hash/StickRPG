...
// Robust tap handler for iOS Safari on HTTPS.
// Fires on touchstart for instant response.
// Guards against double-fire from the synthetic click that follows.
function tap(el, fn) {
if (!el) return;
let active = false;
...
if (active) return; // already fired via touchstart
fn();
});
}

function allocChange(stat, dir) {
if (dir > 0 && allocPts <= 0) return;
if (dir < 0 && allocVals[stat] <= 0) return;
allocVals[stat] += dir;
allocPts       -= dir;
document.getElementById(‘val-’ + stat).textContent = allocVals[stat];
document.getElementById(‘ptsLeft’).textContent = ’Points: ’ + allocPts;
}

tap(document.getElementById(‘btn-sp’), () => allocChange(‘str’,  1));
tap(document.getElementById(‘btn-sm’), () => allocChange(‘str’, -1));
tap(document.getElementById(‘btn-ip’), () => allocChange(‘int’,  1));
tap(document.getElementById(‘btn-im’), () => allocChange(‘int’, -1));
tap(document.getElementById(‘btn-cp’), () => allocChange(‘cha’,  1));
tap(document.getElementById(‘btn-cm’), () => allocChange(‘cha’, -1));

tap(document.getElementById(‘continueBtn’), () => {
...
tap(document.getElementById(‘delSave’), () => {
deleteSaveData();
document.getElementById(‘continueBtn’).style.display = ‘none’;
document.getElementById(‘delSave’).style.display    = ‘none’;
toast(‘Save deleted.’);
});
}
